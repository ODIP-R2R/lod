...
<xsl:variable name="header-title" select="'CMSP Vocabulary Service'"/>
...
<xsl:template match="result" mode="script">
	...
	<script type="text/javascript" src="{$_resourceRoot}scripts/gcis.js"></script>
	<script type="text/javascript">
		$(function() {
			$('.info img')
				.toggle(function () {
			...
			<xsl:if test="$showMap = 'true'">
				...
				<!-- skw --> 
                <xsl:variable name="eastingParam">
                    <xsl:choose>
                        <xsl:when test="$mapParam=''"><xsl:value-of select="$easting"/></xsl:when>
                        <xsl:otherwise><xsl:value-of select="concat($mapParam, '.', $easting)"/></xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                <xsl:variable name="northingParam">
                    <xsl:choose>
                        <xsl:when test="$mapParam=''"><xsl:value-of select="$northing"/></xsl:when>
                        <xsl:otherwise><xsl:value-of select="concat($mapParam, '.', $northing)"/></xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                <!-- skw
...
<xsl:template match="result" mode="header">
	...
	<header>
		<h1><a href="/"><xsl:value-of select="$header-title"/></a></h1>
	</header>
</xsl:template>
